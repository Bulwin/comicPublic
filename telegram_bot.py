...
class ComicBotTelegram:
    ...
    async def _generate_image_and_post(self, query):
        ...
        try:
            ...
            try:
                # Добавляем инструкции для правильной генерации текста
                enhanced_prompt = f"""
{sora_prompt}

КРИТИЧЕСКИ ВАЖНО ДЛЯ ТЕКСТА:
- Используй ТОЧНО тот текст, который указан в описании!
- НЕ переводи и НЕ изменяй указанный текст!
- Воспроизводи текст БУКВАЛЬНО как написано выше
- Текст должен быть чётким и читаемым
- Шрифт в пузырях и подписях - крупный и четкий
"""
